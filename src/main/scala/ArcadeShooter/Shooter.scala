...
class Shooter extends Walker(8, 50, 30) {
  ...
  override def spawn(): List[Bullet] = {
    ...
    toShoot match {
      case 1 => List(new Bullet(x, y - (length / 2), 0, -10, 5))

      case 2 => for (i <- List.range(-2, 4, 2)) yield new Bullet(x, y - (length / 2), i, -10, 5)

      case 3 => for (i <- List.range(0, 21)) yield new Bullet(x, y + (length / 2), (250 * math.cos((i / 20.0) * math.Pi)).toInt / 15, -(250 * math.sin((i / 20.0) * math.Pi)).toInt / 15, 5)
